...
DELETE FROM ogrenciler1 WHERE 
isim = 'Ahmet Ran' OR isim ='Veli Han'

--16-a Tablodaki tüm kayıtları silme : DELETE FROM ...:
SELECT * FROM students;
DELETE FROM students;

--16-b Tablodaki tüm kayıtları silme: TRUNCATE TABLE-DDL ALT GRUBUNDA 
SELECT * FROM doctors;
TRUNCATE TABLE doctors; -- GERİ ALINAMAZ
...
--ref alınan kayıtlar old için silinmez
DELETE FROM calisanlar WHERE id='10002';
--id= 1002 kayıt adresler tarafından ref alındığı için
--FK kısıtlaması silmeye izin verilmez.

DELETE FROM adresler WHERE adres_id = '10002';

--ref alınan kayıt silinerek ilişkiyi koparıldı..
--artık ref olmadığında silmeye izin verir
...
--18-ON DELETE CASCADE: kademeli silme işlemini otomatik olarak gerçekleştirir
...
--Müşteriler tablosundan ürün ismi 'Orange','Apple' veya 'appricot' olan verileri listeleyiniz
...
--Müşteriler tablosundan ürün ismi 'Orange','Apple' veya 'appricot' OLMAYAN verileri listeleyiniz

SELECT * FROM musteriler
WHERE urun_isim NOT IN ('Orange','Apple','Apricot')

--21-BETWEEN.. AND.. komutu
--Müşteriler tablosundan urun_id 20 ile 30(dahil) arasinda olan urunlerin
