...
--NOT: INNER JOIN de ortak olan kayıtları birleştirdiğinden
-- tablo sırası önemli değildir
...
SELECT sirketler.sirket_id, sirket_isim, siparis.sirketler_id, siparis_id,siparis_tarihi
FROM siparis FULL JOIN sirketler
ON sirketler.sirket_id = siparis.sirlet_id
...
SELECT p.isim AS personeller ,m.isim AS yonetici
...
FROM developers
WHERE name LIKE 'E%';
-- E ile başlayıp sonrasında başka karakterler de olabilir
-- ama hepsi büyük harf

SELECT * 
FROM developers
WHERE name ILIKE 'E%';
-- e harfinden sonra büyük/küçük
